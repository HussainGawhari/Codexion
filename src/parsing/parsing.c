...
/*   Updated: 2026/08/18 13:51:53 by hgawhari         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*
    * Parse the scheduler
*/
static bool	parse_scheduler(t_data *data, char *scheduler)
{
	if (ft_strcmp(scheduler, STR_SCHED_FIFO) == 0)
		data->scheduler = FIFO;
	else if (ft_strcmp(scheduler, STR_SCHED_EDF) == 0)
		data->scheduler = EDF;
	else
	{
		print_error(STR_ERR_INPUT_SCHEDULER, scheduler);
		return (false);
	}
	return (true);
}
...
bool	is_valid_input(t_data *data, char **av)
{
	...
	while (i < 8)
	{
		if (!is_positive_integer(av[i]))
			return (print_error(STR_ERR_INPUT_VALUE, av[i]), false);
	...
	if (!parse_data(data, parsed_args))
		return (false);
	return (parse_scheduler(data, av[8]));
}
